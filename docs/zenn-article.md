...
曜日は `M T W R(木) F S(土) U(日)`。手軽ですが、今回の目的には4つ足りません。
...
4. **段階を作れません。** 繰り返しの予定は電源オン／オフで各1組しか持てないので、「予告 → 警告 → 強制終了」のように複数の時刻を並べることができません。
...
**この領域には、標準機能も既製アプリもすでにあります。** 自作の話を始める前に、そこを正直に置いておきます。要件が「決まった時刻に落ちてほしい」だけなら、以下で足ります。

| 選択肢 | できること | 費用 |
| --- | --- | --- |
| Screen Time の Downtime（標準） | 時間帯にアプリの使用を制限する。**電源は落とさない** | 無料 |
| `pmset repeat shutdown`（標準） | 曜日と時刻を指定して落とす。**制約あり（前述）** | 無料 |
| Almighty（Setapp） | Schedule Shutdown。**分・時間のカウントダウン形式** | Setapp に含まれる |
| Mac Shutdown（Tension Software） | Soft / Hard の2モード。Hard はアプリの拒否で中断されにくい | 買い切り |

標準の `pmset` は1行で済みます。制約は[前述のとおり](#標準機能では止められない)で、未保存の書類やターミナルのプロセスがあると落ちません。**「保存していないから落ちない」のでは、止めたい相手が止まりません。**

Screen Time の Downtime はアプリの使用を制限するだけで、電源を落としません。ターミナルでプロセスを走らせたままなら何の妨げにもなりません。

既製アプリのうち Almighty の Schedule Shutdown はカウントダウン形式で、毎日決まった時刻に、という使い方とは別物です。Mac Shutdown は Hard Shutdown を持っていて、これは今回の要件にいちばん近いものです。**買って済むなら、そのほうが早いです。**

（調査は2026年8月時点。各社の仕様は変わるので、判断する前にご自身で確認してください。Mac Shutdown の曜日繰り返しについては筆者は未確認です。）

## それでも書いた理由

自作の動機は「時刻に落とす」ではなく、**止まれない状態を外から止める**ことでした。確認した範囲では、次の4つを備えたものは見つかりませんでした。**「存在しない」ではなく「見つけられなかった」です。** 網羅的に探したわけではありません。

**1. 段階を踏む。** 予告 → 警告 → 最終通告 → 強制終了の4段階。いきなり落とすのではなく、切り上げる判断を発生させるのが目的なので、警告のほうが本体です。実際、導入して効いたのは18:30の警告でした。

**2. スリープ復帰で誤爆しない。** ステージは実行時の時計で決めます。`launchd` は指定時刻にスリープしていると復帰後に遅れて実行するので、「このジョブ＝シャットダウン」と書くと翌朝に蓋を開けた瞬間に落ちます。`pmset` は逆に「スリープ中は落ちない」ので、どちらも設計上の判断が要ります。

**3. 逃げ道を設計に含める。** 当日スキップ（翌日は自動で復活）、今日だけ緩める（延長・レベル変更、翌日は自動で失効）、非常停止（このスクリプトが無くても効く）。**逃げ道が無い道具は結局アンインストールされる**ので、摩擦つきで用意しておくほうが続きます。逆に、緩める手段が「全部スキップ」しか無いと、少し緩めたい日にも全部飛ばすことになります。

**4. 効いているかが見える。** このツールで一番まずいのは、電源が落ちることではなく**効いていないのに効いているつもりでいる**ことです。壊れた設定では走らせず、本番で強制終了まで到達した日時を残し、警告段階の「自動化」の許可が無ければそう出します。

正直に書くと、`pmset` の1行に対してこれは過剰です。**その過剰さに意味があると思えないなら、`pmset` を使ってください。** 実際、段階も逃げ道も要らないなら1行のほうが壊れません。

なお、この手のものをMac App Storeで配布するのは構造的に無理です。App Sandbox が権限昇格を禁じており、`SMJobBless` はサンドボックスアプリから使えたことがなく、App Store は同梱する特権実行ファイル自体のサンドボックス化を要求します。強制シャットダウンの核が root 権限である以上、Developer ID での直販か、ソースを読める形での配布しか道がありません。
